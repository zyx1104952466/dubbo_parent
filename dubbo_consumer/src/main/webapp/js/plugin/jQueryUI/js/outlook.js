...
//	$('#wnav').remove();
	west.removeChild(wnav);
	delete west;
    var div = $("<div id='wnav' class='easyui-accordion' fit='true' border='false'></div>");
    $("#west").append(div);
}
    ...
    if (data.children == null)
        ...
        $.each(data.children, function(i, sm) {
            if (sm.url != null && sm.url != '') {
                ...
                menulist += '<li state="open"><span class="nav">' + sm.name + '</span>'
            }
...
//初始化首页
function InitHome(homepath) {
	if(homepath != ""){
		addTab("欢迎使用",homepath);
	}else{
		addTab("欢迎使用","");
	}
